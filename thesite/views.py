...
def database(request):
    # Get the unique conglomerates to display
    names = Conglomerate.objects.filter(approved_edit=True,).values_list('name')
    ...
    # Now the unique products
    names = Product.objects.filter(approved_edit=True,).values_list('name')
    unique_names = set(names)
    products = []
    for un in unique_names:  # We only want the latest one
        products.append(Product.objects.filter(name=un[0]).order_by('-last_updated').first())
